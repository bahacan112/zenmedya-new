...
export default function CategoryCard({ category }) {
  ...
  const pathSegments = parsedUrl.pathname.split("/"); // URL'yi parçalara ayır
  const kanalId = pathSegments.pop(); // Son elemanı al
    ...
    <div className="category-card">
      <div className="thumbnail">
        <Image
          ...
          src={category.logo}
          alt="category"
          className="thumb-img w-50"
        />
      </div>
      <div className="details">
        <h4 className="title lh-1 mb-0">{category.title}</h4>
        <Link
          href={`/yayinakislari-detail/${kanalId}`}
