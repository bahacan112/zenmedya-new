...
export async function GET(req, { params }) {
  ...
  const playlistUrl = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${params.channel_id}&maxResults=10&key=${apiKey}`;
  try {
    ...
    const trendingOneMoviesData = playlistsData.items.map((playlist) => ({
      ...
      href: playlist.snippet.resourceId.videoId, // Sabit değer
